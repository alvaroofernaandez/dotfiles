...
If it says `Chained PRs recommended: Yes`, `400-line budget risk: High`, estimated PRODUCTION lines exceed 400, or `Decision needed before apply: Yes`, apply cached `delivery_strategy`:

The budget counts production lines only (`additions + deletions` over non-test files); test lines never count toward it. See the Budget Definition in the `chained-pr` skill.
