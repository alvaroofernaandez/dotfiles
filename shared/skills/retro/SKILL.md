...
```bash
_RM="$HOME/.gstack/bin/gstack-retro-metrics"
[ -x "$_RM" ] || _RM=".gstack/bin/gstack-retro-metrics"
...
> **STOP.** Before writing the retrospective narrative (Step 14, after all metrics are computed and compared), Read `~/.gstack/retro/sections/report-format.md` and execute it
