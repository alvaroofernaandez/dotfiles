...
2. **Verify every state, not just resting.** `node $HOME/.claude/skills/design-gates/scripts/verify_states.mjs <file> [--dark]`
   measures default, hover and focus - a button that passes at rest can fail on hover.
3. **One command before reporting done:** `node $HOME/.claude/skills/design-gates/scripts/accuracy_report.mjs`.
...
6. **Responsive is gated too:** `node $HOME/.claude/skills/design-gates/scripts/verify_responsive.mjs <file|dir>` -
