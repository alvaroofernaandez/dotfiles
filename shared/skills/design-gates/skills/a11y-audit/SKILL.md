...
4. **Contrast — measure, don't eyeball.** For rendered HTML, RUN the real-render gates and report their actual output (CLAUDE.md → Verification Protocol): `node $HOME/.claude/skills/design-gates/scripts/measure_render.mjs <file> [--dark]` (every text element) AND `node $HOME/.claude/skills/design-gates/scripts/verify_states.mjs <file> [--dark]` (every interactive element in default/hover/focus — catches hover-state failures). For loose color pairs, `python3 $HOME/.claude/skills/design-gates/scripts/contrast.py "<fg>" "<bg>"`. Never state a ratio you did not measure.
